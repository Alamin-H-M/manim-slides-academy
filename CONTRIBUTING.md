...
2. **Keep it lightweight.** The whole site is ~2.5 MB. New videos: 854×480 @ 15 fps,
   re-encoded with x264 CRF 26 (see `generator/` notes below), ideally < 50 KB each.
...
- `generator/` — Python generator: `common.py` (template + highlighter) and one
  `page_*.py` per page. Run `python3 generator/build.py` to regenerate `site/*.html`.
...
python3 generator/encode_videos.py           # re-encodes media/videos/... into site/assets/video/
# 4. reference it in the right generator/page_*.py via vid("MyScene", '''<code>''')
# 5. regenerate:
